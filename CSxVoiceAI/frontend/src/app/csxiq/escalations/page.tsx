...
export default function EscalationsPage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Waiting list */}
      <div className="w-[352px] flex-shrink-0 border-r border-slate-200 bg-white flex flex-col">
        <div className="px-5 pt-5 pb-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-[17px] font-semibold text-slate-900 tracking-tight">Live Escalations</h1>
          </div>
          <p className="text-slate-500 text-[13px] mt-1">Voice handoffs from the CSxAI agent — claim within the accept window</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[13.5px] font-medium text-slate-900 truncate">{e.contactName}</span>
                  <span className={cn('px-1.5 py-px rounded-md text-[11px] font-medium border', pm.cls)}>{pm.label}</span>
                </div>
                <p className="text-[12.5px] text-slate-500 mt-0.5 truncate">{e.scenario}</p>
        ...
        <div className="max-w-3xl mx-auto px-8 py-7">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-slate-900 tracking-tight">{selected.contactName}</h2>
          ...
          <div className="mt-3 flex items-center gap-2 text-[12px]">
            ...
            <span className="text-slate-400">Sentiment: {selected.context.sentiment}</span>
          </div>
          ...
          <div className="mt-5 bg-white border border-slate-200 rounded-xl p-4">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">AI Handoff Summary</p>
              <span className="ml-auto text-[10.5px] text-slate-300">Read before you join</span>
            </div>
            <p className="text-[13.5px] text-slate-700 leading-relaxed">{selected.aiSummary}</p>
          </div>

          {/* Live transcript */}
          <div className="mt-4">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">Conversation so far</p>
          ...
          <div className="mt-6">
            {!joined ? (
              <div className="bg-white border border-emerald-300 rounded-xl p-4 flex items-center justify-between shadow-sm shadow-emerald-100/50">
                <div>
                  <p className="text-[13.5px] font-medium text-slate-900">Member is on the line with the AI</p>
                ...
                <button
                  ...
                  className={cn('px-4 py-2.5 rounded-lg text-[13px] font-semibold flex items-center gap-2 transition-colors',
                    accept > 0 ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-slate-100 text-slate-400 cursor-not-allowed')}
                >
              ...
              <div className="bg-white border border-emerald-300 rounded-xl p-4 shadow-sm shadow-emerald-100/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13.5px] font-medium text-slate-900">You’re live with {selected.contactName}</span>
                  </div>
