...
const channelIcon: Record<Channel, any> = { whatsapp: MessageSquare, web: Globe, email: Mail, voice: Phone }
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Inbox */}
        <div className="w-[288px] flex-shrink-0 border-r border-[#E9E9E7] bg-[#FBFBFA] flex flex-col">
          <div className="px-4 py-2.5">
            <p className="text-[11px] font-medium text-[#9B9A97]">Inbox · Omnichannel</p>
          </div>
            ...
            {threads.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[12.5px] text-[#787774] truncate mt-0.5">{t.preview}</p>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-3">
            {active.messages.map((m, i) => (
              <div key={i} className={cn('flex flex-col max-w-[68%]', m.from === 'csr' ? 'ml-auto items-end' : 'items-start')}>
                <div className={cn('rounded-2xl px-3.5 py-2 text-[13.5px] leading-relaxed',
          ...
          <div className="p-4 border-t border-[#E9E9E7]">
            <div className="flex items-center gap-2 bg-white border border-[#E9E9E7] rounded-xl px-3 py-1.5 focus-within:ring-2 focus-within:ring-[#37352F]/10 focus-within:border-[#D6D5D2] transition">
              ...
              />
              <button onClick={send} disabled={!draft.trim()} className="w-7 h-7 rounded-lg bg-[#37352F] flex items-center justify-center hover:bg-[#2A2925] transition-colors flex-shrink-0 disabled:opacity-40">
        ...
        <div className="w-[300px] flex-shrink-0 border-l border-[#E9E9E7] bg-[#FBFBFA] flex flex-col">
          ...
          <div className="p-3 border-b border-[#E9E9E7]">
            ...
            {active.copilotSuggestion ? (
              <div className="bg-white border border-[#E9E9E7] rounded-xl p-3">
                <p className="text-[13px] text-[#37352F] leading-relaxed">{active.copilotSuggestion}</p>
                {active.copilotSource && (
                  <div className="flex items-center gap-1.5 mt-2.5 text-[11px] text-[#9B9A97]">
                    <BookOpen className="w-3 h-3" />
                    <span className="truncate">{active.copilotSource}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 mt-3">
                  <button
                    onClick={() => setDraft(active.copilotSuggestion || '')}
                    className="flex-1 px-3 py-1.5 rounded-lg bg-[#37352F] text-white text-[12.5px] font-medium hover:bg-[#2A2925] transition-colors flex items-center justify-center gap-1.5"
                  >
                    <CornerDownLeft className="w-3.5 h-3.5" /> Insert into reply
                  </button>
                </div>
          ...
          {/* Team presence */}
          <div className="px-4 py-2.5">
            <p className="text-[11px] font-medium text-[#9B9A97]">Team</p>
          </div>
          <div className="flex-1 overflow-y-auto px-2">
            {TEAM.map((m) => (
              <div key={m.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-md hover:bg-[#F1F0EE] transition-colors">
                <div className="relative">
                  ...
                  <span className="absolute -bottom-0.5 -right-0.5 ring-2 ring-[#FBFBFA] rounded-full">
                ...
                {m.activeCount > 0 && (
                  <span className="text-[10.5px] text-[#787774] bg-[#F1F0EE] rounded px-1.5 py-px tabular-nums">{m.activeCount}</span>
                )}
              </div>
            ))}
          </div>
